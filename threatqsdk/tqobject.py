...
class ThreatQuotientObject:
    ...
    def add_comment(self, value):
        ...
        res = self.tq.post(self._get_api_endpoint() + "/comments?with=sources", data=data)
    ...
    def add_attribute(self, name, value, modify=False, sources=None, published_at=None):
        ...
        if modify:
            if not self.current_attributes:
                res = self.tq.get(f"{self._get_api_endpoint()}/attributes")
            ...
            for attr in self.current_attributes:
                if attr["name"] == name and attr["value"].lower() != value.lower():
                    ...
                    self.tq.delete(f"{self._get_api_endpoint()}/attributes/{attribute_id}")
                    # break
        ...
        res = self.tq.post(f"{self._get_api_endpoint()}/attributes", data=data)
