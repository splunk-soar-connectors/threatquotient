...
class Event(ThreatQuotientObject):
    ...
    def add_spearphish(self, text):
        ...
        self.tq.post("/api/events/%i/spearphish" % self.eid, data)  # noqa: UP031
    ...
    def upload(self, sources=None):
        ...
        if self.typename == "Spearphish":
            if not self.text or not self.statusname or not self.title:
                raise ValueError("Must provide spearphish text, default Indicator Status, and Title (Subject).")
            ...
            data = {
                ...
                "globals": {
                    ...
                    "relations": {"adversaries": [], "events": [], "attachments": [], "indicators": [], "signatures": []},
                },
            }
            result = self.tq.put("/api/events/import/%i?with=attachments,events" % eid, data)  # noqa: UP031
            if not result or "data" not in result or "id" not in result["data"]:
                raise exceptions.UploadFailedError(result)

            result = self.tq.get("/api/events/import/%i?with=attachments,events" % eid)  # noqa: UP031
            if not result or "data" not in result or "id" not in result["data"]:
                raise exceptions.UploadFailedError(result)

            otherid = result["data"]["events"][0]["id"]
            data = {"title": self.title, "happened_at": self.happened_at, "description": self.desc}
            result = self.tq.put("api/events/import/%i/events/%i" % (eid, otherid), data=data)  # noqa: UP031
            if not result or "data" not in result or "id" not in result["data"]:
                raise exceptions.UploadFailedError(result)

            result = self.tq.get("/api/events/import/%i/commit" % eid)  # noqa: UP031
