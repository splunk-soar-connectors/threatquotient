...
class IndicatorParser(object):
    ...
    regex_map = {
        ...
        'url': r'(((?<=(?:^))|(?<=(?:\s))|(?<=(?:>))|(?<=(?:\()))(?P<scheme>\w{3,6}(?:(?:(?:(?::)|(?:\[:\]))\/\/)|(?:\[:\/\/\]))(?P<authority>(?:(?<=(?::\/\/))|(?<=(?:\[:\]\/\/))|(?<=\s)|(?<=\[:\/\/\]))(?P<auth>[a-zA-Z0-9-]+:[a-zA-Z0-9_\-~`!$%^&*\(\)=|\+{}\\<>,\.:;]+@)?(?:(?:(?P<subdomain>[a-zA-Z0-9_-]{1,25}(?:(?:\.)|(?:\[\.\]))){1,5})(?P<tld>[a-zA-Z0-9-]{2,25})|(?P<ipv4>((?:(?:1?[0-9]{1,2}|2[0-4][0-9]|25[0-5])(?:(?:\[\.\])|(?:\.))){3}(?:1?[0-9]{1,2}|2[0-4][0-9]|25[0-5]))))(?P<port>:\d{2,5})?)(?:(?P<path>\/[a-zA-Z0-9_\-~`!@#$%^&*\(\)=|\+{}\\,\.;:\[\]]+(?<!\))(?<!\.|:)){1,})?(?P<query>\/?\?(?:(?:%[0-9a-f][0-9a-f]|[-a-z0-9\._~\w\^(?:[\.\])!$\&\'\(\)\*\+,;=:@])|[\w\/\?])*(?<!\))(?<!\.|:))?(?P<fragment>\#(?:(?:%[0-9a-f][0-9a-f]|[-a-z0-9\._~\w(?:[\.\])!$\&\'\(\)\*\+,;=:@\^])|[\/\?])*(?<!\))(?<!\.|:))?))',  # NOQA: E501
        ...
        'ip': r'\b((?:(?:1?[0-9]{1,2}|2[0-4][0-9]|25[0-5])(?:(?:\[\.\])|(?:\.))){3}(?:1?[0-9]{1,2}|2[0-4][0-9]|25[0-5]))\b',  # noqa:E501
        'cve': r'\bcve-\d{4}-\d{4,7}\b',
        'filename': r'\b[a-zA-Z0-9\-.[\]_]+(\.|\[\.\])(exe|src|vbs|rtf|doc|xls|docx|xlsx|zip|rar|7z|jar)\b',
        # This will not pick out unix-based paths
        'filepath': r'((?!\\r\\n)((?<=(?:^))|(?<=(?:\s))|(?<=(?:>)))(?P<disk>(?:%\w+%)|(?:[A-Z]:))?(?!\/\w? )(?P<start_name>(?:\\|\\\\)[a-zA-Z0-9_ \-{}\.-]+){1}(?:(?P<separator>\\|\\\\)(?<! \/)(?P<path>[a-zA-Z0-9_ \-{}\.-]+)){0,}(?P<end_name>(?:\\|\\\\)(?<! \/)[a-zA-Z0-9_\-{}\.-]+))'  # noqa:E501
    }
