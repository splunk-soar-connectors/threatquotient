...
class ThreatQConnector(BaseConnector):
    ...
    def query_indicators(self, params):
        ...
        for index, item in enumerate(items):
            ...
            self.save_progress(f"Querying for [{item}] - {index + 1}/{len(items)}")
            try:
            ...
            msg = f"ThreatQ found [{len(details)}] result(s)"
    ...
    def create_indicators(self, params):
        ...
        self.save_progress(f"Parsed [{len(found)}] indicators; Unable to parse [{len(unknown)}] strings")
        ...
        msg = f"Successfully uploaded [{len(uploaded)}] indicator(s)"
    ...
    def create_task(self, params):
        ...
        # Create task payload
        data = {"name": name, "priority": priority, "status_id": THREATQ_TASK_STATUS_MAP.get(status, 1), "sources": [source_obj.to_dict()]}
        ...
        try:
            ...
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(msg)
            action_result.set_status(phantom.APP_ERROR, f"{THREATQ_ERROR_UPLOAD_TASK}. {error_message}")
            return action_result
        ...
        indicators = []
        if found:
            self.save_progress(f"Uploading [{len(found)}] indicators")
        for item in found:
            obj = ThreatQObject(self.tq, "indicators")
            obj.fill_from_api_response(item)
        ...
        if uploaded_inds:
            self.save_progress(f"Relating [{len(uploaded_inds)}] indicators to the task")

        failed_count = 0
        for i in uploaded_inds:
            try:
                self.tq.post(f"{i._get_api_endpoint()}/tasks", data=[{"id": res["id"]}])
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                msg = f"{THREATQ_ERROR_RELATE_INDICATOR_TO_TASK.format(i)}. {error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                failed_count += 1
        ...
        if event.oid:
            ...
            try:
                ...
                msg = f"{error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                action_result.set_status(phantom.APP_ERROR, THREATQ_ERROR_RELATE_CONTAINER_EVENT_TO_TASK.format(error=error_message))
                return action_result
        ...
        res.update({"host": self.tq.threatq_host, "indicators": [ind._to_dict() for ind in uploaded_inds], "api_name": "tasks"})
    ...
    def create_event(self, params):
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(f"Error occurred while parsing start time. {msg}")
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(f"Error occurred while parsing due time. {msg}")
            due_time = None
        ...
        try:
            ...
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(msg)
            action_result.set_status(phantom.APP_ERROR, f"{THREATQ_ERROR_UPLOAD_EVENT}. {error_message}")
            return action_result
        ...
        indicators = []
        if found:
            self.save_progress(f"Uploading [{len(found)}] indicators and relating to event")
        for item in found:
            obj = ThreatQObject(self.tq, "indicators")
            obj.fill_from_api_response(item)
            obj.status = self.default_status
    ...
    def upload_spearphish(self, params):
        ...
        try:
            ...
            self.debug_print(
                "Error occurred while reading the file in text mode ('r'). Hence, switching to binary mode ('rb') to read the contents of file."
            )
        ...
        if event.oid:
            ...
            try:
                self.tq.post(f"/api/events/{spearphish.eid}/events", data=data)
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                msg = f"{error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                action_result.set_status(phantom.APP_ERROR, THREATQ_ERROR_RELATE_CONTAINER_EVENT_TO_SPEARPHISH.format(error=error_message))
                return action_result

        # Add in some extra info to the output data
        try:
            data = spearphish._to_dict()
            data.update({"id": spearphish.eid, "api_name": "events", "host": self.tq.threatq_host})
    ...
    def upload_file(self, params):
        ...
        if event.oid:
            ...
            try:
                self.tq.post(f"/api/attachments/{tq_file.fid}/events", data=data)
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                msg = f"{error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                action_result.set_status(phantom.APP_ERROR, THREATQ_ERROR_RELATE_CONTAINER_EVENT_TO_FILE.format(error=error_message))
                return action_result

        # Add in some extra info to the output data
        data = {}
        data.update({"id": tq_file.fid, "api_name": "files", "name": tq_file.name, "host": self.tq.threatq_host})
    ...
    def start_investigation(self, params):
        ...
        try:
            ...
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(msg)
            action_result.set_status(phantom.APP_ERROR, f"{THREATQ_ERROR_CREATE_INVESTIGATION}. {error_message}")
            return action_result
        ...
        for ind in uploaded_inds:
            ...
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                msg = f"{THREATQ_ERROR_RELATE_INDICATOR_TO_INVESTIGATION.format(ind)}. {error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                failed_count += 1
        ...
        if event.oid:
            ...
            try:
                ...
                msg = f"{error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                action_result.set_status(phantom.APP_ERROR, THREATQ_ERROR_RELATE_CONTAINER_EVENT_TO_INVESTIGATION.format(error=error_message))
                return action_result
    ...
    def create_adversaries(self, params):
        ...
        self.save_progress(f"Creating [{len(items)}] adversaries in ThreatQ")
        ...
        msg = f"Successfully uploaded [{len(uploaded)}] adversaries"
    ...
    def create_custom_objects(self, params):
        ...
        self.save_progress(f"Creating [{len(to_handle)}] objects in ThreatQ")
        ...
        for objs in objects.values():
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                msg = f"{THREATQ_ERROR_BULK_UPLOAD.format(error=error_message)} -- {traceback.format_exc()}"
                self.debug_print(msg)
                continue

            uploaded.extend([u_obj for u_obj in objs if u_obj.oid])
            self.save_progress(f"Successfully uploaded [{len(uploaded)}] {objs[0].api_name} objects")
        ...
        # Save progress
        if len(uploaded) == 0:
            action_result.set_status(phantom.APP_ERROR, "No {} created in ThreatQ".format(obj_data.get("display_name_plural")))
        else:
            action_result.set_status(phantom.APP_SUCCESS, msg)
    ...
    def add_context(self, params, context_type, context_cb):
        ...
        for key, value in params.items():
            if key == context_type or key.startswith(f"{context_type}_"):
        ...
        self.save_progress(f"Adding {context_type} to [{len(to_handle)}] objects in ThreatQ")
        ...
        msg = f"Successfully added {context_type}(s) to [{len(uploaded)}] objects"
        ...
        self.save_progress(msg)
        if len(uploaded) == 0:
            action_result.set_status(phantom.APP_ERROR, f"No {context_type}(s) created in ThreatQ")
        else:
            action_result.set_status(phantom.APP_SUCCESS, msg)
    ...
    def get_related_objects(self, params):
        ...
        for index, item in enumerate(items):
            ...
            try:
                related_res = self.tq.get(f"/api/{base_obj}/{result[0].oid}/{related_obj}", withp="attributes").get("data", [])
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                msg = f"{error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
            ...
            msg = f"ThreatQ found [{len(related_objects)}] result(s)"
            self.save_progress(msg)
    ...
    def create_signature(self, params):
        ...
        try:
            ...
            msg = f"{error_message} -- {traceback.format_exc()}"
            self.debug_print(msg)
            action_result.set_status(phantom.APP_ERROR, f"{THREATQ_ERROR_UPLOAD_SIGNATURE}. {error_message}")
            return action_result
    ...
    def set_indicator_status(self, params):
        ...
        # Upload the objects
        self.save_progress(f"Uploading [{len(indicators)}] indicators")
        try:
            ThreatQObject.bulk_upload(self.tq, indicators)
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
        ...
        if uploaded:
            self.save_progress(f"Setting status of [{len(uploaded)}] indicators to [{indicator_status}]")
        ...
        for i in uploaded:
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                msg = f"{THREATQ_ERROR_SET_INDICATOR_STATUS.format(i)}. {error_message} -- {traceback.format_exc()}"
                self.debug_print(msg)
                failed_count += 1
        ...
        msg = f"Successfully set status of [{len(uploaded)}] indicator(s)"
    ...
    def query_object_details(self, object_type, value, exact=False, relationships=False):
        ...
        # Send the request
        res = self.tq.get(f"/api/{object_type}", params=params)
        ...
        for i in res.get("data", []):
            ...
            if relationships:
                for obj_type in object_types:
                    if "_" in obj_type:
                        i[obj_type] = self.tq.get("/api/{}/{}/{}".format(object_type, i["id"], obj_type)).get("data", [])
    ...
    def handle_action(self, params):
        ...
        if not action:
            return self.set_status(phantom.APP_ERROR, f"No action handler associated with action [{action_id}]")
